...
import com.oneconnect.avssolution.ui.SideBarActivity
import es.dmoral.toasty.Toasty
import kotlinx.android.synthetic.main.oneconnect_main.drawer_layout
...
abstract class ActivityOneConnect : SideBarActivity() {
    ...
    var icon_too: ImageView? = null
    ////////////////////////

    var bg_changed: Boolean? = false
    var appName_: TextView? = null


    var powerBtn1: LinearLayout? = null
    var powerBtn2: LinearLayout? = null
    var statusConnected: TextView? = null
    var ipAddresss: TextView? = null


    var btn_next_activity: Button? = null

    var downloadSpeedTv: TextView? = null
    var downloadUnitTv: TextView? = null
    var uploadSpeedTv: TextView? = null
    var uploadUnitTv: TextView? = null
    var speedLayoutInternet: LinearLayout? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        appName_ = findViewById(R.id.app_name)

        powerBtn1 = findViewById(R.id.power_btn1)
        powerBtn2 = findViewById(R.id.power_btn2)
        statusConnected = findViewById(R.id.status_connected)
        ipAddresss = findViewById(R.id.ip_address)

        downloadSpeedTv = findViewById(R.id.download_speed)
        downloadUnitTv = findViewById(R.id.download_unit)
        uploadSpeedTv = findViewById(R.id.upload_speed)
        uploadUnitTv = findViewById(R.id.upload_unit)

        speedLayoutInternet = findViewById(R.id.speed_layout)


        btn_next_activity = findViewById(R.id.btn_next_activity)

        dl = findViewById(R.id.dl)
        navigation = findViewById(R.id.nav_left)
        icon_too = findViewById(R.id.icon_tool)


        icon_too?.setOnClickListener {
        ...
        navigation?.setNavigationItemSelectedListener { menuItem ->
            ...
            when (menuItem.itemId) {
               R.id.nav_left_rate_app->{
                    openPlayStoreListing(this)
                   dl?.closeDrawer(Gravity.LEFT);

               }

                R.id.nav_left_share_app->{
                      shareApp(this)
                    dl?.closeDrawer(Gravity.LEFT);

                }

                R.id.nav_left_privacy->{
                    val url = getString(R.string.privicy_policy)
        ...
        appName_?.setText(getString(R.string.app_name))
    ...
    fun shareApp(context: Context) {
        ...
        val shareBodyText = "http://play.google.com/store/apps/details?id="+packageName
...
//                btnPower2!!.visibility = View.GONE
                ipAddresss!!.visibility = View.GONE
                statusConnected?.setText("Disconnected")
                powerBtn1!!.visibility = View.VISIBLE
                powerBtn2!!.visibility = View.GONE

                speedLayoutInternet!!.visibility = View.GONE
...
//                rockrtButton!!.visibility = View.VISIBLE

                speedLayoutInternet!!.visibility = View.VISIBLE
                ipAddresss!!.visibility = View.VISIBLE
                statusConnected?.setText("You are connected")
                powerBtn1!!.visibility = View.GONE
                powerBtn2!!.visibility = View.VISIBLE
...
//                rockrtButton!!.visibility = View.GONE
                speedLayoutInternet!!.visibility = View.GONE
...
//        btnbg3?.setBackgroundResource(R.drawable.connect_btn3)
        ...
        if(bg_changed == true) {
            ...
            backgrounds[0] = ResourcesCompat.getDrawable(resources, R.drawable.img, null)
            backgrounds[1] = ResourcesCompat.getDrawable(resources, R.drawable.img, null)
...
//        Log.d("mylog","upload "+byteinKb)
//        Log.d("mylog","Download"+byteoutKb)

        val uploadSpeed = byteoutKb.split(" ").toTypedArray()[1]
        val uploadUnit = byteoutKb.split(" ").toTypedArray()[2]

        Log.d("mylog","DownloadSpead"+uploadSpeed + "uit "+uploadUnit )
        uploadSpeedTv?.setText(uploadSpeed)
        uploadUnitTv?.setText(uploadUnit)


        val downloadSpeed = byteinKb.split(" ").toTypedArray()[1]
        val downloadUnit = byteinKb.split(" ").toTypedArray()[2]
        Log.d("mylog","DownloadSpead"+downloadSpeed + "uit "+downloadUnit )
        downloadSpeedTv?.setText(downloadSpeed)
        downloadUnitTv?.setText(downloadUnit)
